...
Status SearchList(LinkedList L, ElemType e) {
	ElemType i;                              //定义一个整型变量用于进行后续循环 
	for(i=1;i<e&&L->next!=NULL;i++){         //通过循环判断L能否指向第e个结点 
	L=L->next;	                        
	}
...
Status IsLoopList(LinkedList L) {
	...
	ElemType k=999;                      //k用来存储两个指针数据相同时的数据，便于判断指针数据相同时对应的数都是k 
	p1=p2=(LinkedList)malloc(sizeof(LNode));     //开拓空间 
	p1=L;
	p2=L->next;
	while(1){
	p1=p1->next;
	if(p2->next!=NULL) p2=p2->next->next;   //在走两步之前判断能不能走两步 
	else return ERROR;
